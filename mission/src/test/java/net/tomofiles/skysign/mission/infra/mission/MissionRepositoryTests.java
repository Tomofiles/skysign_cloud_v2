...
import static net.tomofiles.skysign.mission.infra.mission.RecordObjectMother.newNormalCarbonCopiedMissionRecord;
...
public class MissionRepositoryTests {
    ...
    public void getMissionByIdTest() {
        when(missionMapper.find(DEFAULT_MISSION_ID.getId()))
                .thenReturn(newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION));
    ...
    public void getAllMissionsTest() {
        when(missionMapper.findAll())
                .thenReturn(Arrays.asList(new MissionRecord[] {
                        newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION),
                        newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION),
                        newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION)
                }));
        when(waypointMapper.find(DEFAULT_MISSION_ID.getId()))
        ...
        assertAll(
            ...
            () -> assertThat(missions.get(0).isCarbonCopy()).isEqualTo(expectedMission.isCarbonCopy()),
    ...
    /**
     * リポジトリーからカーボンコピーでないオリジナルのMissionエンティティをすべて取得する。
     */
    @Test
    public void getAllOriginalMissionsTest() {
        when(missionMapper.findAllOriginal())
                .thenReturn(Arrays.asList(new MissionRecord[] {
                        newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION),
                        newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION),
                        newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION)
        ...
        List<Mission> missions = repository.getAllOriginal();
    ...
    /**
     * リポジトリーからカーボンコピーでないオリジナルのMissionエンティティをすべて取得する。<br>
     * エンティティが存在しない場合、空リストが返却されることを検証する。
     */
    @Test
    public void getAllOriginalNoMissionsTest() {
        List<Mission> missions = repository.getAllOriginal();
    ...
    public void saveNewMissionTest() {
        repository.save(newSingleNavigationCarbonCopiedMission(
                DEFAULT_MISSION_ID,
                DEFAULT_VERSION,
                DEFAULT_GENERATOR.get()));

        verify(missionMapper, times(1)).create(newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION));
    ...
    public void savePreExistMissionTest() {
        ...
        verify(missionMapper, times(1)).update(newNormalCarbonCopiedMissionRecord(DEFAULT_MISSION_ID, DEFAULT_VERSION));
